...
int main(int argc, char *argv[]) 
{
    ...
    if (tag_num == 0) {
        ...
        sprintf(buffer, "Z id%d: started", pro_num);
        ...
        if (list->shared_data->office.is_open == 1) {
            ...
            sprintf(buffer, "Z id%d: entering for a service %d", pro_num, service);
            SM_CounterPrint(list->shared_data, buffer);

            // goes to front with service type <n>
            SM_OfficeService(list->shared_data, pro_num, service);
        } 

        // customer is going home
        sprintf(buffer, "Z id%d: going home", pro_num);
        SM_CounterPrint(list->shared_data, buffer);
    }
    ...
    if (tag_num == 1) {
        ...
        // print process started
        sprintf(buffer, "U id%d: started", pro_num);
        SM_CounterPrint(list->shared_data, buffer);
        ...
        while (list->shared_data->office.is_open == 1 || list->shared_data->office.sem_1_count > 0 
            ...
            SM_OfficeServe(list->shared_data, pro_num, arg_tu);
        }

        // officer is going home
        sprintf(buffer, "U id%d: going home", pro_num);
        SM_CounterPrint(list->shared_data, buffer);
    }
